...
public class PreguntasCentro {
	...
	//a
		public Double PromedioEdadProfesoresFuncional(String dni) {
			Set<Grupo> matriculasAlumno = matriculas().todas().stream().filter(m -> m.dni().equals(dni)).map(m -> m.grupo()).collect(Collectors.toSet());
			Set<String> dniProfesoresAlumno = asignaciones().todas().stream().filter(a -> matriculasAlumno.contains(a.grupo())).map(a -> a.dni()).collect(Collectors.toSet());
			return profesores().todos().stream().filter(p -> dniProfesoresAlumno.contains(p.dni())).mapToDouble(p -> Period.between(p.fechaDeNacimiento().toLocalDate(), LocalDate.now()).getYears()).average().orElse(0);
		}
		
		public Double PromedioEdadProfesoresImperativo(String dni) {
			Set<Grupo> matriculasAlumno = new HashSet();
			for(Matricula matricula : matriculas().todas()) {
				if(matricula.dni().equals(dni)) {
					matriculasAlumno.add(matricula.grupo());
				}
			}
			Set<String> dniProfesoresAlumno = new HashSet();
			for(Asignacion asignacion : asignaciones().todas()) {
			...
			for(Profesor profesor: profesores().todos()) {
		...
		public Grupo grupoMayorDiversidadEdadFuncional() {
			Map<Grupo, List<Matricula>> grupoAmatriculas = matriculas().todas().stream().collect(Collectors.groupingBy(Matricula::grupo));
		...
		public Double difEdadIFuncional(List<Matricula> ls) {
			return ls.stream().mapToDouble(m -> Period.between(alumnos().alumno(m.dni()).fechaDeNacimiento().toLocalDate(), LocalDate.now()).getYears()).max()
					.orElse(0) - ls.stream().mapToDouble(m -> Period.between(alumnos().alumno(m.dni()).fechaDeNacimiento().toLocalDate(), LocalDate.now()).getYears()).min().orElse(0);
		}
		...
		public Double difEdadImperativo(List<Matricula> ls) {
			...
			for(Matricula matricula:ls) {
				edades.add((double) Period.between(alumnos().alumno(matricula.dni()).fechaDeNacimiento().toLocalDate(), LocalDate.now()).getYears());
			}
		...
		public Alumno alumnoMasMatriculasFuncional() {
			return matriculas().todas().stream().collect(Collectors.groupingBy(Matricula::dni, Collectors.counting())).entrySet().stream()
					.max(Map.Entry.comparingByValue()).map(e -> alumnos().alumno(e.getKey())).orElse(null);
		}
		...
		public String rangosEdadPorAlumnoImperativo(String s) {
			...
			for(Alumno alumno:alumnos().todos()) {
		...
		public String nombreProfesorMasGruposFuncional(int min, int max) {
			...
			return asignaciones().todas().stream()
					.filter(a -> Period.between(profesores().profesor(a.dni()).fechaDeNacimiento().toLocalDate(), LocalDate.now()).getYears() >= min && 
							Period.between(profesores().profesor(a.dni()).fechaDeNacimiento().toLocalDate(), LocalDate.now()).getYears() <= max)
					.collect(Collectors.groupingBy(Asignacion::dni, Collectors.counting())).entrySet().stream()
					.collect(Collectors.toMap(e -> profesores().profesor(e.getKey()).nombreCompleto(), e -> e.getValue())).entrySet().stream()
		...
		public String nombreProfesorMasGruposImperativo(int min, int max) {
			...
			for(Asignacion asignacion: asignaciones().todas()) {
				Integer edad = Period.between(profesores().profesor(asignacion.dni()).fechaDeNacimiento().toLocalDate(), LocalDate.now()).getYears();
				if(edad <= max && edad >= min) {
					profesorNumGrupos.merge(profesores().profesor(asignacion.dni()).nombreCompleto(), 1, Integer::sum);
				}
			}
			String key = null;
			Integer value = Integer.MIN_VALUE;
		...
		public List<String> nombresAlumnosMayorNotaFuncional(Integer n, Integer a){
			...
			return alumnos().todos().stream().sorted(Comparator.comparingDouble(Alumno::nota).reversed()).filter(al -> al.fechaDeNacimiento().getYear()>a).limit(n).map(al -> al.nombre()).toList();
		}
		...
		public static void main(String[] args) {
			Centro c = Centro.of();
			System.out.println(c.PromedioEdadProfesoresImperativo("54561899B"));
			System.out.println(c.PromedioEdadProfesoresFuncional("54561899B"));
			System.out.println(c.grupoMayorDiversidadEdadImperativo());
			System.out.println(c.grupoMayorDiversidadEdadFuncional());
			System.out.println(c.alumnoMasMatriculasFuncional());
			System.out.println(c.alumnoMasMatriculasImperativo());
			System.out.println(c.nombresAlumnosMayorNotaFuncional(5, 2001));
			System.out.println(c.nombresAlumnosMayorNotaImperativo(5, 2001));
			System.out.println(c.nombreProfesorMasGruposFuncional(21, 24));
			System.out.println(c.nombreProfesorMasGruposImperativo(21, 24));
			System.out.println(c.rangosEdadPorAlumnoImperativo("20 - 23, 24 - 26"));
			System.out.println(c.rangosEdadPorAlumnoFuncional("20 - 23, 24 - 26"));
